...
class LarvalFishElement(Lagrangian3DArray):
    ...
    variables = Lagrangian3DArray.add_variables([
        ...
        ('stage_fraction', {'dtype': np.float32,  # to track percentage of development time completed
                            'units': '',
                            'default': 0.}),
        ('hatched', {'dtype': np.uint8,  # 0 for eggs, 1 for larvae - in this case putting directly 1 should move towards the larvae phase
                     'units': '',
                     'default': 1}),
        ('length', {'dtype': np.float32,
                    'units': 'mm',
                    'default': 0.170}), # 0.170 Rodriguez-Perez (others 0.190 Robert, 0.162 FAO data mean size liberation ostrea)  
        ('weight', {'dtype': np.float32,
                    'units': 'mg',
                    'default':  0.000266}), #from allometry  
...
class LarvalFish(OceanDrift):
    ...
    required_variables = {
        ...
        'sea_floor_depth_below_sea_level': {'fallback': 35}, # max depth gebco
        'ocean_vertical_diffusivity': {'fallback': 0.00001, 'profiles': True},
        'ocean_mixed_layer_thickness': {'fallback': 20}, # northern adriatic sea 
        'sea_water_temperature': {'fallback': 20, 'profiles': True}, #adriatic mean late spring - always a value provided no need for fallback
        'sea_water_salinity': {'fallback': 38, 'profiles': True} #adriatic
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._add_config({
            'IBM:fraction_of_timestep_swimming':
                {'type': 'float', 'default': 0.5, #assumption of half time 
    ...
    def update_terminal_velocity(self, Tprofiles=None,
        ...
        #  properties that determine buoyancy
        larvasize = self.elements.diameter  # 0.0016 for ostrea 
        larvasalinity = self.elements.neutral_buoyancy_salinity
        ...
        DENSlarv = self.sea_water_density(T=T0, S=larvasalinity)
        dr = DENSw - DENSlarv  # density difference
        ...
        W = (1.0 / my_w) * (1.0 / 18.0) * g * larvasize ** 2 * dr

        # check if we are in a Reynolds regime where Re > 0.5
        highRe = np.where(W * 1000 * larvasize / my_w > 0.5)
    ...
    def fish_growth(self, weight,temperature):
        ...
        # Daily growth rate in percent 
        #basato su dati perez
        GR= 0.4752* np.exp(0.149*temperature) 
        g= GR / 100. * self.time_step.total_seconds()/(60*60*24) 
        return weight * g
    
    def update_fish_larvae(self):

        # Hatching of eggs - in this case since they are released already as larvae default hatched=1 
        ...
        self.elements.length[larvae] = 0.0541*np.log(w) + 0.6154
        self.elements.diameter[larvae] = self.elements.length[larvae]/1000
    ...
    def larvae_vertical_migration(self):
        ...
        T = self.environment.sea_water_temperature[larvae]

        swim_speed = 0.0017 - 0.007*L - 0.00009*T + 0.0006*L*T  #based on Rodriguez-Perez work with different temperatures and sizes 
        ...
        # although a phase shift of 1-2 hours should not make much difference
         
        if self.time.hour > 17 & self.time.hour < 6:
            direction = 0.6  # Swimming up when light is decreasing - reducing the swim speed up by putting 0.6 instead of 1 given  Rodriguez-Perez more swim down
        else:
            direction = -1 #swimming down when light increasing
